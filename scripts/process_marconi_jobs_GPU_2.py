...
"""
def read_data(jobfile_single, jobfile_multi, pluginpath):    
    ...
    df_total_power = pd.concat([pd.read_parquet(x).assign(gpu=y) for x, y in zip(power_filepaths, range(NB_GPUS))]).reset_index(drop=True)
...
def filter3_3(df_jobs_single, exclusive_job_ids, joined_total_power):
    ...
    df_total_power_exclusive_single = pd.DataFrame(result, columns=["timestamp", "value", "node", "gpu", "job_id"])  
...
def check_condition_f3(x, exclusive_job_ids=None):
    #print(x)
    # x0=timestamp, x1=value, x2=node_id, x3=gpu, x4=job_ids
    return np.any(pd.Series(x[4].split(",")).isin(exclusive_job_ids).values)
...
Save results to csv
"""
def save_results(df_exclusive_jobs_single, df_exclusive_jobs_multi, df_total_power_exclusive_single, df_total_power_exclusive_multi, jobfile_single, pluginpath):
    ...
    df_total_power_exclusive_single.to_csv(pluginpath+metric+"a_0_filter123_singlenode.csv", index=False)
    df_total_power_exclusive_multi.to_csv(pluginpath+metric+"a_0_filter123_multinode.csv", index=False)


"""
Run workflow
"""
def run_workflow(pluginpath, jobfile_single, jobfile_multi):
    df_jobs_single, df_jobs_multi, df_total_power = read_data(jobfile_single, jobfile_multi, pluginpath)
    ...
    save_results(df_exclusive_jobs_single, df_exclusive_jobs_multi, df_total_power_exclusive_single, df_total_power_exclusive_multi, jobfile_single, pluginpath)
    ###############################
...
if __name__ == '__main__':
    ...
    run_workflow(args.pluginpath, args.jobfilesingle, args.jobfilemulti) 